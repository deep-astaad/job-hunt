    ...
    if batch_size > 0:
        for item in items:
            try:
                ...
                # Increment redis pending jobs and total jobs counter
                r.incr(f"pipeline:{pipeline_run_id}:pending_jobs")
                ...
                chain(
                    ...
                    rank_job_multi_profile.s(profiles=ranker_profiles, pipeline_run_id=pipeline_run_id),
                ).apply_async()
...
def run_pipeline(actor_configs, profile_ids):
    ...
    r.set(f"pipeline:{pipeline_run_id}:pending_jobs", 0)
